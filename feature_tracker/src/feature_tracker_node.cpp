...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg) {
  ...
  if (img_msg->header.stamp.toSec() - last_image_time > 1.0 ||
    ...
    VINS_WARN("[FT] image discontinue! reset the feature tracker!");
  ...
  if (PUB_THIS_FRAME) {
    pub_count++;
    // === DIAG: feature tracking statistics ===
    {
      int total_pts = 0, long_track_pts = 0;
      for (int i = 0; i < NUM_OF_CAM; i++) {
        total_pts += trackerData[i].cur_pts.size();
        for (unsigned int j = 0; j < trackerData[i].track_cnt.size(); j++) {
          if (trackerData[i].track_cnt[j] >= 5)
            long_track_pts++;
        }
      }
      double img_dt = img_msg->header.stamp.toSec() - last_image_time +
                      (img_msg->header.stamp.toSec() - last_image_time);
      // Actually use the simple interval
      double frame_interval = 1.0 / FREQ; // expected
      VINS_LOG("[FT] tracked=%d | long_track(>=5)=%d (%.1f%%) | img_t=%.3f",
               total_pts, long_track_pts,
               total_pts > 0 ? 100.0 * long_track_pts / total_pts : 0.0,
               img_msg->header.stamp.toSec());
    }
    ...
    ROS_DEBUG("publish %f, at %f", feature_points->header.stamp.toSec(),
              ros::Time::now().toSec());
    // === DIAG: published feature count ===
    VINS_LOG("[FT] published_features=%lu", feature_points->points.size());
  ...
  VINS_LOG("[FT] frame_process_time=%.1fms", t_r.toc());
}
...
int main(int argc, char **argv) {
  ...
  // 初始化文件日志 → ~/catkin_vins/logs/vins_ft_*.log
  VinsFileLogger::instance().init("ft");
