...
void Estimator::processImage(
  ...
  VINS_LOG(
      ...
      marginalization_flag == MARGIN_OLD ? "KEYFRAME (margin_old)"
                                         : "NON-KEYFRAME (margin_2nd_new)",
      f_manager.getFeatureCount(), f_manager.last_track_num);
...
void Estimator::optimization() {
  ...
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    ...
    problem.SetManifold(para_Pose[i], new PoseLocalParameterization());
#else
  ...
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ...
    problem.SetManifold(para_Ex_Pose[i], new PoseLocalParameterization());
#else
    ceres::LocalParameterization *local_parameterization =
        new PoseLocalParameterization();
    problem.AddParameterBlock(para_Ex_Pose[i], SIZE_POSE,
  ...
  if (relocalization_info) {
    ...
    problem.SetManifold(relo_Pose, new PoseLocalParameterization());
#else
    ceres::LocalParameterization *local_parameterization =
        new PoseLocalParameterization();
